...
public final class DrawingPanel extends FileFilter
    ...
    */
    ...
    public Color getColorAtPixel(int x, int y) {
    	...
    	if (x < 0 || x >= canvasImage.getWidth()) { 
    		throw new IndexOutOfBoundsException("The requested x value of " + x 
    					+ " is outside the image's coordinate range!"
    					+ "\nx must be between 0 and " + (canvasImage.getWidth() - 1));
    	}
    	if (y < 0 || y >= canvasImage.getHeight()) { 
    		throw new IndexOutOfBoundsException("The requested y value of " + y 
    					+ " is outside the image's coordinate range!"
    					+ "\ny must be between 0 and " + (canvasImage.getHeight() - 1));
    	}
